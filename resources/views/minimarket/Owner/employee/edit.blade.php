        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('minimarket.owner.employee.update', ['id' => $user->id]) }}" method="POST">
                        @csrf
                        @method('put')

                        <div class="max-w-xl">
                            <x-input-label for="name" value="Nama" />
                            <x-text-input id="name" type="text" name="name" class="mt-1 block w-full" value="{{ old('name',  $user->name) }}" required autofocus/>
                        ...
                        <div class="mb-4 max-w-xl">
                            ...
                            <x-text-input id="password" type="password" name="password" class="mt-1 block w-full" value="{{ old('password') ?? $user->password }}" required/>
                        ...
                        <div class="mb-4 max-w-xl">
                            ...
                            <x-select-input id="branch_id" name="branch_id" class="mt-1 block w-full" required>
                                @foreach($branches as $key => $value)
                                    @if(old('branch_id',  $user->branch_id) == $key)
                                        <option value="{{ $key }}" selected>{{ $value }}</option>
                                    @else
                                        <option value="{{ $key }}">{{ $value }}</option>
                                    @endif
